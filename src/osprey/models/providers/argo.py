    ...
    def execute_completion(
        ...
        **kwargs
        ...
        if output_format is not None:
            # Try structured outputs first, fall back to JSON mode if it fails
            try:
                ...
                # If parsed is None, the model didn't return valid JSON
                if result is None:
                    # Try to get the raw content for debugging
                    raw_content = response.choices[0].message.content
                    logger.error(f"ARGO structured output parsing failed. Full raw content:\n{raw_content}")
                    
                    # Check if content has markdown code blocks (common issue)
                    if raw_content and "```" in raw_content:
                        logger.warning("Detected markdown code blocks in structured output - falling back to JSON mode")
                    
                    # Fall through to JSON mode fallback
                    raise ValueError("Structured output returned None")

                # Handle TypedDict conversion
                if is_typed_dict_output and result is not None and hasattr(result, 'model_dump'):
                    return result.model_dump()
                return result
                
            except (openai.LengthFinishReasonError, ValueError, openai.BadRequestError, Exception) as e:
                # Fallback to JSON mode with manual parsing
                error_msg = str(e)
                logger.warning(f"ARGO structured output failed ({type(e).__name__}), falling back to JSON mode")
                ...
                fallback_messages[-1] = {"role": "user", "content": message + json_instruction}
                ...
                try:
                    ...
                    logger.error(f"JSON mode request failed: {json_mode_error}")
                ...
                logger.debug("ARGO PROVIDER: Starting JSON cleanup process")
                logger.debug(f"JSON mode fallback - Full raw response:\n{raw_json}")
                ...
                if not cleaned_json.startswith('{') and not cleaned_json.startswith('['):
                    logger.warning(f"Response doesn't start with JSON. First 200 chars: {cleaned_json[:200]}")
                    # Try to find where JSON actually starts
                    json_start = cleaned_json.find('{')
                    if json_start > 0:
                        logger.info(f"Found JSON starting at position {json_start}, stripping prefix text")
                ...
                logger.debug(f"BEFORE FIX: {cleaned_json[:200]}")
                ...
                cleaned_json = cleaned_json.replace(',True', ',true')
                cleaned_json = cleaned_json.replace(',None', ',null')
                logger.debug(f"AFTER FIX: {cleaned_json[:200]}")
