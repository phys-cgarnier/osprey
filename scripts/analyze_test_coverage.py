...
def print_overall_summary(coverage_data: dict):
    ...
    if coverage_pct >= target:
        print("\n✅ Target coverage achieved! Ready for Phase 1 refactoring.")
    elif coverage_pct >= target * 0.9:
        print("\n🟡 Almost there! Just a bit more to reach target.")
...
def compare_to_baseline(coverage_data: dict):
    ...
    if diff > 0:
        print(f"\n✅ Coverage improved by {diff:.1f}%! Great work!")
    elif diff < 0:
        print(f"\n⚠️  Coverage decreased by {abs(diff):.1f}%. Review recent changes.")
    else:
...
def main():
    ...
    print("📊 Analyzing existing coverage data (not running tests)...\n")
